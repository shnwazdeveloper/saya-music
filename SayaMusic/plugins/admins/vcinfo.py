...
from SayaMusic.utils.database import group_assistant
from SayaMusic.utils.admin_filters import admin_filter
...
async def vc_info(client, message: Message):
    ...
    try:
        ...
        for p in participants:
            ...
            screen_status = "" if getattr(p, "screen_sharing", False) else ""
            volume_level = getattr(p, "volume", "N/A")

            info = f"{mute_status} {name} |  {volume_level}"
            if screen_status:
